...
class test:
    def open(self, wt):
        # print(wt)
        dc = 0.0
        print('shutter will open for '+str(wt)+' seconds')
        wavegen.generate(device_data, channel=1, function=wavegen.function.square, offset=2.5, frequency=1e03,
                         amplitude=2.5, symmetry=dc, wait=0.0, data=[])
        time.sleep(wt)

    def close(self, wt):
        # print(wt)
        dc = 80
        print('shutter will close for '+str(wt)+' seconds')
        wavegen.generate(device_data, channel=1, function=wavegen.function.square, offset=2.5, frequency=1e03,
                         amplitude=2.5, symmetry=dc, wait=0.0, data=[])
        time.sleep(wt)
...
class grafit(Frame):
    ...
    def plotit(self):
        df = open(r'C:\Users\skyphysics\Downloads\ShutterTestData\Test4_Data', 'a')  # open csv for writing - add file path
        ...
        if self.is_running == True :
            ...
            #print(volt_1)
            #print(volt_2)
            v = 18.98 - volt_2 #- volt_1
            #print('voltage: ', v)
            volt = v / 6.0  # current through sol, divide by resistance of L (6 ohms)
            #print('current: ', volt)

            #f = open(self.txt_doc.get() + ".txt", "a")
            #f.write(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + "," + volt + '\n')
            #f.close()

            writer.writerow(['[' + datetime.datetime.now().strftime("%m/%d/%Y %H:%M") + '] ' + str(volt)])
            ...
            if len(self.schedule.queue) == 1:
                self.cycles += 1
                for i in range (0,10):
                    cycle_period = (66+128)*10+10800
                    self.schedule.enter(cycle_period*self.cycles+ i*(66+128), 1, self.mytest.open, [66.0])  # start time, repeat, function, run time
                    self.schedule.enter(cycle_period*self.cycles + 66+ i*(66+128), 1, self.mytest.close, [128.0])
                self.schedule.enter(cycle_period*self.cycles + 10*(66 + 128), 1, self.mytest.close, [10800.0])
    ...
    def __init__(self):
        ...
        for i in range(0, 10):
            cycle_period = (66 + 128) * 10 + 10800
            self.schedule.enter(cycle_period * self.cycles + i*(66+128), 1, self.mytest.open, [66.0])
            self.schedule.enter(cycle_period * self.cycles + 66 + i*(66+128), 1, self.mytest.close, [128.0])
        self.schedule.enter(cycle_period * self.cycles + 10 * (66 + 128), 1, self.mytest.close, [10800.0])
        print('number of scheduled events '+str(len(self.schedule.queue)))
        self.plotit()
